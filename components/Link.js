...
const Link = (props) => {
  ...
  const saveData = () => {
    if (value) {
      props.setContent([...props.content, { type: "link", data: value }]);
      setValue("");
    }
  };

  return (
    <div className="w-full border border-border px-2 py-2 rounded flex mb-2">
      <svg
        className="text-muted me-3 fill-primary"
        width="24"
        height="24"
        viewBox="-5.5 0 32 32"
        version="1.1"
      ...
      <input
        ...
        value={value}
        type="text"
        className="border-none w-full outline-none bg-transparent text-primary text-lg"
      />
    </div>
  );
};
